...
-- Setup lazy.nvim
require("lazy").setup({
	-- detect shiftwidth, tabstop and expandtab heuristically
	"tpope/vim-sleuth",

	-- git signs for visually managing changes (eg. in gutter)
	-- :help gitsigns
	{
	...
	-- display pending keybinds on keypresses
	{
		...
		event = "VimEnter",
		opts = {
			-- how long to wait before (re)opening which-key
			delay = 0,
			icons = {
			...
			spec = {
				...
				{ "<leader>sk", ":DevdocsOpenCurrent<cr>", desc = "Search DevDocs (current)" },
				{ "<leader>sK", ":DevdocsOpen<cr>", desc = "Search DevDocs (all)" },
			},
		},
	},
	...
	-- :Telescope help_tags
	{
		...
		dependencies = {
			...
			-- ..
			{
				...
				-- what to run on update/install
				build = "make",
				-- should it be installed or loaded?
				cond = function()
		...
		config = function()
			...
			vim.keymap.set("n", "<leader>s?", builtin.keymaps, { desc = "Search keymaps" })
	...
	--
	-- BEGIN UNMODIFED FROM KICKSTART SECTION
	--
	...
	--
	-- END UNMODIFED FROM KICKSTART SECTION
	--

	{
		...
		opts = {
			filetypes = {
				...
				typescript = { "html", "javascript", "typescript" },
				svelte = { "html", "css", "javascript", "typescript", "svelte" },
			},
		},
	},
